...
timeout 180 ssh "${SSH_OPTS[@]}" -p "$PORT" root@"$HOST" \
  "pgrep -f 'uvicorn lip_service' >/dev/null && echo '已有在跑的服务，跳过启动' || { cd $REMOTE_DIR && OMP_NUM_THREADS=16 setsid nohup /root/miniconda3/bin/python -m uvicorn lip_service:app --host 0.0.0.0 --port 8002 > /root/lip_server.log 2>&1 < /dev/null & sleep 36; echo '已启动'; }" 2>&1 | tail -2
H=$(timeout 30 ssh "${SSH_OPTS[@]}" -p "$PORT" root@"$HOST" "curl -s -m 6 http://127.0.0.1:8002/health" 2>/dev/null || true)
echo "  云侧 health: ${H:-（无响应）}"
echo "$H" | grep -q '"status":"ok"' || { echo "  ✗ 云服务未就绪 → ssh 上去看 /root/lip_server.log"; exit 3; }

say 4 "建本机隧道（本机 8002 → 云 8002）"
if pgrep -f "ssh -N -L 8002" >/dev/null; then
  echo "  已有隧道，跳过"
else
