...
class BrainOutput:
    ...
    # ⚠️ v1.10（依 ADR-008）：切句后**送 TTS 前**须经 brain/text_clean.clean_for_tts 清理 ——
    #    剥离 Markdown/emoji/编号列表/链接/舞台说明，数学符号转口语；清理后为空则跳过该句不合成。
    #    同一 token 流仍**原样**下发前端显示（气泡按 Markdown 渲染）：「给人看的文本」与
    #    「给 TTS 念的文本」只在 TTS 入口分离，清理结果不回写。
...
### 5.4 语音识别上行（ASR chunk）

```
POST /api/v1/asr/chunk
请求: { "session_id": "xxx", "audio_pcm_16k_b64": "<base64 PCM16 单声道 16kHz>", "seq": 0, "end": false }
响应: { "seq": 0, "text": "本片增量", "full_text": "累计文本", "is_final": false, "start_ms": 0, "end_ms": 600 }
```

> v1.9 补齐：此前该端点只有数据契约（§2 `AsrChunk`），**HTTP 契约缺失**，而前端一直在用。

**分片节奏**：前端每 ~600ms 一片（`asr/streaming.py CHUNK_STRIDE`；`useMicCapture` 按 9600 样本切）。

**`seq` 语义（v1.9 明确；此前只存在于代码）**：

- `seq` 从 **0 开始、每片递增**（句内序号）；前端在每轮 `start()` 时重置为 0。
- ⚠️ **`seq == 0` 被后端视为「新语句起点」，会丢弃该会话已有流式状态并重建识别器。**
  若破坏递增（例如恒发 0），每片都重建 → **识别只反映最后一片（~600ms）**，
  表现为「识别莫名变差 / 只认出末尾几个字」。
  实测复现：正常递增 → `你们家运费怎么算`；恒发 `seq=0` → `嗯`（仅末片结果）。

**端点检测不在后端**（ADR-004）：`end=true` 由前端静音判定或用户手动结束触发；
后端只做识别，并在 `end=true` 后释放该会话的流式状态。

**错误**：`ASR_ERROR` 503（base64 解码失败为 400）；`ASR_TIMEOUT` 503（单块 >3s）。

**调试采集（非契约）**：后端以 `ASR_DEBUG=1` 启动时，每次 `end=true` 会把该句**完整音频 + 识别文本**
落盘到 `backend/eval/reports/asr_capture/`，供真人麦克风准确率复盘（`eval/verify_asr_mic_accuracy.py`）。
默认关闭。
...
| **v1.10** | **2026-09-19** | **§2「Brain → TTS」补实现约定：切句后送 TTS 前经 `brain/text_clean.clean_for_tts` 清理（剥离 Markdown/emoji/编号/链接/舞台说明，数学符号转口语），清理后为空则跳过该句；同一 token 流原样下发前端显示。**依据 ADR-008**（未清理时 TTS 会念出星号/emoji；实测噪音样本清理后 4 处纯噪音句被跳过）。同日：对话人格外置为 `config.yaml` `llm.persona`（`teacher` 默认 / `cs` 可切），见 ADR-007** | - |
| **v1.9** | **2026-09-17** | **补 §5.4 语音识别上行契约**（`POST /asr/chunk` 此前只有数据契约、无 HTTP 端点文档，实现早已在用）；**明确 `seq` 语义与破坏后果**（`seq==0` 重建识别器 → 识别退化为只认最后一片，实测可复现）；记录 `ASR_DEBUG` 采集开关（非契约，供真人准确率复盘） | - |
