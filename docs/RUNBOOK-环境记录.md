...
**④ "第二轮的回答开头重复上一轮内容" —— 麦克风把扬声器回放收进了 ASR**

症状：第一轮问「能听到吗？」，第二轮提问时数字人**开头又答了一遍「能听到的」**。

**后端侧已排除**（实测）：第二轮 TTS 的 `start_ms` 从 **0** 重新开始（没接着上一轮），词文本全是本轮内容
（无「能听到」），`done.answer` 也是本轮问题。串的不是后端。

真因：按下「说话」时**只开麦克风、没让数字人闭嘴**。若数字人还在播（或播尾音），麦克风就把扬声器里的
上一轮回答收进去——`echoCancellation: true` 在**外放/大音量**下消不干净——ASR 把旧内容识别成本轮输入
→ LLM 照着再答一遍。

修法（`onMicToggle`）：开麦前 ① `stopPlayback()`（**无条件**：状态可能已回 `listening`，但前端播放队列
里仍有余音）② `state === 'speaking'` 时 `handleInterrupt()`（语义上按「说话」本就该打断）。

> 这是**外放 + AEC 不足**的**第二种**表现（第一种是本节 ① 的自打断）。两者指向同一结论：
> **AEC 是这套打断设计成立的前提**。外放场景必须解决（戴耳机 / 回声门控 / ADR-004 的 P2 VAD 迁后端）。

**⑤ 外放时"语音无缘无故说一半就停" —— 自打断的处置（2026-09-22 加开关）**

症状：用户**外放**（笔记本扬声器）提问，数字人答到一半突然停，气泡后缀「（已被打断）」；
用户原话是"生成的语音会无缘无故的中断，不会完整的说完整个回答"。

**三步定位（先分清是 TTS 断了还是被"打断"掐了）**：

```bash
LOG="$LOCALAPPDATA/Temp/dh_backend.log"
grep -c "interrupt" "$LOG"        # 有 /interrupt 请求 → 是"打断"路径，不是 TTS 出错
grep "\[tts\]" "$LOG" | tail      # 送合成里有整句 → TTS 侧没断，是播放被切断
```
再问一句**外放还是耳机**：外放 = 扬声器声被麦克风收进去（AEC 近场消不干净）→ 音量远超判据
（`max(底噪×8, 0.03)`，连续 2 帧）→ **数字人被自己的声音打断**。判据本质是"有足够大的声音"，
不是"听懂你在插话"，所以它分不清你的声音和它自己的回声。

**处置：对话页右上「允许插话打断」开关**（默认开；外放时取消勾选，戴耳机可保持开启）。

- 持久化 key：`localStorage['dh.barge.enabled.v1']`（`'1'`/`'0'`）
- 关掉只影响**自动**打断：手动「让她停下」与开麦打断不受影响（能力不丢）
- 实现：`useDuplexSession` 的 `wantBarge = bargeEnabled && state === 'speaking' && !recording`

**气泡后缀现在能分辨是谁打断的**（此前一律写"已被打断"，用户无法自证是否环境音误触发）：
自动插话「（检测到插话，停下了）」/ 手动「（你让她停下了）」/ 开麦「（你开始提问）」。

**验证口径**（无麦克风的 headless 也能量化）：包装 `navigator.mediaDevices.getUserMedia` 计数，
开关开→进入 speaking 时调用 **1** 次（barge 监听真的开了）、关→**0** 次（不再开监听）。
带 `--use-fake-device-for-media-stream --use-fake-ui-for-media-stream` 跑 headless 即可复现。
