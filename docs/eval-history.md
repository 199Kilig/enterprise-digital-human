...
| 2026-09-23 | 资源 | 口型 CPU 融合吞吐（**去 PIL 化后**） | **~215 fps（4.65 ms/帧）** | 同一 25 帧批次三实现对照：官方 PIL **16.54 ms/帧** → 纯 numpy **4.65 ms/帧（3.6×）**；torch GPU 版 15.76 ms/帧（**1.0×**，瓶颈为 PCIe 搬运非算力）；新旧实现**逐像素 maxdiff = 0**（2026-09-23） | 云 GPU（AutoDL 4090 24G） | backend/eval/reports/lip_rtf_optimization.md |
| 2026-09-23 | 延迟 | 口型端到端（服务进程内，**去 PIL 化后**） | **711 ~ 737 ms / RTF 0.72 ~ 0.74**（1s 音频 25 帧） | 同上第 40 行口径；分解 audio ~10 + gen ~300 + blend ~116 + 编码 ~338；**RTF 由 1.115 降至 <1 → ADR-006「头号补测项」结清**（2026-09-23） | 云 GPU（AutoDL 4090 24G） | backend/eval/reports/lip_rtf_optimization.md |
| 2026-09-23 | 延迟 | 端到端（含口型链路，**真实对话**） | **`lip_video` 事件到达跨度 11.8 s**；浏览器 `createObjectURL` 与事件到达**仅差 3 ms** | 浏览器 CDP 钩 `EventSource.addEventListener` + `URL.createObjectURL`；本机 socket 同口径对照 12.7 s。**「~80 秒缺口」随 RTF 改善自行消失，真因为上游队列背压（非前端缺陷）**（2026-09-23） | 本机 + 云 GPU | backend/eval/verify_lip_stream_timeline.py |
