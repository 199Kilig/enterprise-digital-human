...
| 检查项（v1.2 实测修正）          | 目标                                          |
| :------------------------------- | :-------------------------------------------- |
| 单块处理延迟（墙钟）             | ≤300ms（**实测 166ms ✓**，CPU RTF 0.27）     |
| 首字端到端延迟                   | ≈ 块粒度 + 单块处理 ≈ 766ms（600ms/块粒度下"首字端到端"不可能 ≤300ms，原目标仅对单块处理成立） |
| 增量输出频率                     | ≥每 500ms 一次（**实测 600ms/块，每块均有增量 ✓**） |
| `is_final`语义正确               | 最后一句为True，中间为False（**✓**）          |
| VAD端点正确触发                  | 调用方逻辑触发（P2 实现）；本脚本验证 is_final 传递语义 ✓ |
...
**通过标准（v1.3 实测回填）**：
- 首 token 目标 ≤500ms ⚠️ **实测 503~1108ms 波动（均值 ~740ms，deepseek-chat，3 次）** → 建议值放宽至 ≤1200ms（2026-09-02）
- 事件结构包含 `choices[0].delta.content` 字段（✓）
- 支持正常 `[DONE]` 结束（✓，35 chunk）
...
| **v1.2** | **2026-09-02** | **V-03 实测回填：首字延迟口径修正（600ms/块粒度下单块处理 166ms ✓，端到端首字≈766ms，原 ≤300ms 仅对单块处理成立）；API 用法对齐官方（chunk_size=[0,10,5]、cache 跨块传递、numpy float32 输入）** | -    |
| **v1.3** | **2026-09-02** | **V-04 实测回填：LLM 首 token 实测 503~1108ms 波动（均值 ~740ms），≤500ms 目标放宽至 ≤1200ms** | -    |
